import { PROFILE_INTERACTIONS, type ProfileInteraction } from '../features/analytics/client';
import { isFirstPartyRequest, recordAnalyticsEvent } from '../features/analytics/events.server';
import { cloudflare } from '../lib/cloudflare';
import type { Route } from './+types/analytics-profile-interaction';

export async function action(args: Route.ActionArgs) {
  if (!isFirstPartyRequest(args.request)) throw new Response('Forbidden', { status: 403 });

  const contentLength = Number(args.request.headers.get('Content-Length') ?? '0');
  if (contentLength > 256) throw new Response('Bad request', { status: 400 });

  const body = await args.request.json().catch(() => null);
  const interaction =
    body && typeof body === 'object' ? (body as { interaction?: unknown }).interaction : null;
  if (
    typeof interaction !== 'string' ||
    !PROFILE_INTERACTIONS.includes(interaction as ProfileInteraction)
  ) {
    throw new Response('Bad request', { status: 400 });
  }

  const { env } = args.context.get(cloudflare);
  const profile = await env.DB.prepare(
    `SELECT p.id
       FROM profiles p
       JOIN workspaces w ON w.id = p.workspace_id
      WHERE w.slug = ?1 AND p.visibility = 'public'`,
  )
    .bind(args.params.slug)
    .first<{ id: string }>();
  if (!profile) throw new Response('Not found', { status: 404 });

  const result = await recordAnalyticsEvent(env, args.request, {
    profileId: profile.id,
    type: 'profile_interaction',
    interaction,
  });
  return new Response(null, {
    status: 204,
    headers: result.setCookie ? { 'Set-Cookie': result.setCookie } : undefined,
  });
}
