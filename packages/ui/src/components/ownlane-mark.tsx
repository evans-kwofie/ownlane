type OwnlaneMarkProps = {
  /**
   * `open` is the primary mark: a stroked outline that holds up next to text.
   * `folded` is the solid, tighter form for small or busy surfaces.
   */
  variant?: 'folded' | 'open';
  className?: string;
};

export function OwnlaneMark({ variant = 'open', className }: OwnlaneMarkProps) {
  if (variant === 'folded') {
    return (
  ...
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 40 40" fill="none">
      <path
        d="M12 5h16l7 7v16l-7 7H12l-7-7V15"
        stroke="currentColor"
        strokeWidth="5"
